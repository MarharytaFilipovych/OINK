...
class FunctionGenerator:
    ...
    def generate_standalone_function(self, node, visitor):
        self.__prepare_function_context(node.variable, node.return_type, node.params)
    ...
    def generate_member_function(self, struct_name: str, node, visitor):
        ...
        self.__prepare_function_context(mangled_name, node.return_type, node.params, is_member=True, struct_name=struct_name)
    ...
    def __process_lambda_call(self, node, visitor) -> str:
        ...
        for i, arg in enumerate(node.arguments):
            ...
            expected_llvm_type = self.__get_llvm_type(expected_type)
        ...
        return self.__emit_function_call(return_type, return_llvm_type, lambda_func_name, args)

    def __process_regular_function_call(self, node, visitor) -> str:
        return_type = self.type_converter.get_node_type(node)
        return_llvm_type = self.__get_llvm_type(return_type)
        ...
        expected_types = [DataType.I32] * len(node.arguments) \
            if not func_info else [self._string_to_type(t) for t in func_info.param_types]
        args = [self.__build_call_argument(arg, visitor, expected_type)
                for arg, expected_type in zip(node.arguments, expected_types)]
        return self.__emit_function_call(return_type, return_llvm_type, func_name, args)
    ...
    def generate_member_function_call(self, node, visitor) -> str:
        ...
        return_type, return_llvm_type, expected_types, mangled_name = self.__get_call_info(obj_type, node.value)
    ...
    def __generate_member_to_member_call(self, node, visitor) -> str:
        struct_name = self.current_struct_context
        return_type, return_llvm_type, expected_types, mangled_name = self.__get_call_info(struct_name, node.value)
    ...
    def __prepare_function_context(self, func_name: str, return_type, params, is_member: bool = False, struct_name: Optional[str] = None):
    ...
    def __declare_function_params(self, node):
        for param in node.params:
            ...
            param_llvm_type = self.__get_llvm_type(param_type)
    ...
    def __build_call_argument(self, arg, visitor, expected_type: Union[DataType, str]) -> str:
        arg_value = arg.accept(visitor)
        arg_type = self.type_converter.get_node_type(arg)
        expected_llvm_type = self.__get_llvm_type(expected_type)
